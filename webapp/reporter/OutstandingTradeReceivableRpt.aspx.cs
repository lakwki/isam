...
{
    public partial class OutstandingTradeReceivableRpt : com.next.isam.webapp.usercontrol.PageTemplate
    {
        ...
        private void initControls()
        {
            ...
            uclProductTeam.initControl(UclSmartSelection.SelectionList.productCode);
        ...
        private ReportClass genReport()
        {
            ...
            OutstandingTradeReport rpt = ARAPReportManager.Instance.getARReport(cutoffDate, invoiceDateFrom, invoiceDateTo, invoiceUploadDateFrom, invoiceUploadDateTo,
                budgetYear, periodFrom, periodTo, currencyId, baseCurrencyId, exchangeRateYear, exchangeRatePeriod, payRefCode, purchaseTermId, orderType, 
                officeId, officeIdList, handlingOfficeId, productTeamList, vendorId,
                isSZOrder, isUTOrder, isOPROrder, customerTypeCollector, tradingAgencyCollector, sampleOrderGroup, int.Parse(this.radDataType.SelectedValue), int.Parse(this.rad_ReportVersion.SelectedValue), this.LogonUserId);
        ...
        protected void btn_Submit_Click(object sender, EventArgs e)
        {
            ...
            if (rpt != null)
                ReportHelper.export(rpt, HttpContext.Current.Response, CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, "OutstandingReceivableRpt");
            else
            {
                ...
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "OSReceivableReport", "alert('This Exchange Rate is not ready for reporting purpose.\\nPlease select the other exchange rate period.\\nThe latest available Exchange Rate is " + period + ".');", true);
            }
        }
        ...
        protected void btn_Export_Click(object sender, EventArgs e)
        {
            ...
            ReportClass rpt = genReport();
            if (rpt != null)
                ReportHelper.export(rpt, HttpContext.Current.Response, CrystalDecisions.Shared.ExportFormatType.ExcelRecord, "OutstandingReceivableRpt");
            else
            {
                ExchangeRateDef latestExchangeRate = NssWorker.Instance.getExchangeRateDefByDateLatest(ddl_BaseCurrency.selectedValueToInt, ExchangeRateType.PAYABLE_RECEIVABLE.Id, DateTime.Now);
                AccountFinancialCalenderDef latestRatePeriod = CommonUtil.getAccountPeriodByDate(9, latestExchangeRate.EffDateFrom);
