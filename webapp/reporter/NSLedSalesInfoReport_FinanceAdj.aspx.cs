...
{
    public partial class NSLedSalesInfoReport_FinanceAdj : com.next.isam.webapp.usercontrol.PageTemplate
    {
        ...
        private int getNoOfWeekByPeriodColumn(int periodCol)
        {
            ...
            if (period == 1 || period == 4 || period == 7 || period == 10 || period == 12)
                return 5 + noOfAdjCols;
            else
                return 4 + noOfAdjCols;
        }
        ...
        private int getPeriodFirstWeekColumn(int period)
        {
            ...
            /*
            int noOfAdjCols = 1;
            return periodCol - getNoOfWeekByPeriodColumn(periodCol) - noOfAdjCols;
            */
            return periodCol - getNoOfWeekByPeriodColumn(periodCol);

        }

        private int getPeriodLastWeekNo(int period)
        {
            int weekNo = 0;
            for (int i = 1; i <= period; i++)
            {
                if (i == 1 || i == 4 || i == 7 || i == 10 || i == 12)
                    weekNo += 5;
                else
                    weekNo += 4;
            }
        ...
        List<int> summaryCol = new List<int>() { 0, 13, 19, 25, 32, 38, 44, 51, 57, 63, 70, 76, 83 };

        private void updateBookedStockValue(SpreadsheetDocument document, string sheetId, int startingRow, string paramVal)
        {
            string v = OpenXmlUtil.getCellString(document, sheetId, startingRow + 10, 2);

            if (v == string.Empty)
                v = "=";

            OpenXmlUtil.setCellValue(document, sheetId, startingRow + 10, 2, v + string.Format("+{0}", paramVal), CellValues.Number);
        }

        private void updateBookedSettlementDiscountValue(SpreadsheetDocument document, string sheetId, int startingRow, string paramVal)
        {
            string v = OpenXmlUtil.getCellString(document, sheetId, startingRow + 11, 2);

            if (v == string.Empty)
                v = "=";

            OpenXmlUtil.setCellValue(document, sheetId, startingRow + 11, 2, v + string.Format("+{0}", paramVal), CellValues.Number);
        }
        ...
        private void genReportOpenXml()
        {
            ...
            */
            ...
            string sourceFileName = "NSLedSalesInfo_FinanceAdj - AdjCol.xlsm";
            ...
            int cumCol = OpenXmlUtil.getExcelColumnNo("CF");
            int lifeCol = OpenXmlUtil.getExcelColumnNo("CG");
            int sizeTableStartCol = OpenXmlUtil.getExcelColumnNo("CI");
            int profitCol = OpenXmlUtil.getExcelColumnNo("CO");
            int forecastCol = OpenXmlUtil.getExcelColumnNo("CR");
            ...
            foreach (NSLedProfitabilitiesDef def in profitList)
            {
                ...
                if (def.ItemNo == "A16139")
                    def.TotalSettlementDiscount = (0.02m * (def.TotalFOBCost));
                ...
                */
                ...
                // get the time when the item cost was booked in epicor
                List<int> shippedPeriodList = AccountManager.Instance.getNSLedItemShippedPeriodList(def.ItemNo, def.SeasonId);
                ...
                if (bfSales.Count() > 0)
                {
                    ...
                    // assumed BF cost data has been adjusted
                    ...
                    firstStockCol = bfCol;
                    this.updateBookedStockValue(document, currentSheetId, itemStartingRow, string.Format("{0}{1}", OpenXmlUtil.getColumnLetter(bfCol), (itemStartingRow + 11).ToString()));
                    this.updateBookedSettlementDiscountValue(document, currentSheetId, itemStartingRow, string.Format("{0}{1}", OpenXmlUtil.getColumnLetter(bfCol), (itemStartingRow + 12).ToString()));
                ...
                else
                {
                    ...
                    if (def.ShippedYear == -1 || def.ShippedYear >= fiscalYear) // only shipped in current year, set b/f values as zero
                    {
                ...
                if (startWeek == 0) //No sales
                {
                    for (int i = 1; i <= fiscalWeek; i++)
                    {
                        ...
                        do
                        {
                            ...
                            //if ((def.ShippedYear == fiscalYear && currentCol == this.getPeriodFirstWeekColumn(CommonUtil.getFiscalPeriodByFiscalWeek(i)) && shippedPeriodList.Contains(fiscalYear * 100 + CommonUtil.getFiscalPeriodByFiscalWeek(i))) || (def.ShippedYear == fiscalYear && currentCol == this.getPeriodColumn(CommonUtil.getFiscalPeriodByFiscalWeek(i)) && shippedPeriodList.Contains(fiscalYear * 100 + CommonUtil.getFiscalPeriodByFiscalWeek(i)))) //  first week of shipped period & shipped period columns
                            if ((currentCol == this.getPeriodFirstWeekColumn(CommonUtil.getFiscalPeriodByFiscalWeek(i)) && shippedPeriodList.Contains(fiscalYear * 100 + CommonUtil.getFiscalPeriodByFiscalWeek(i))) || (currentCol == this.getPeriodColumn(CommonUtil.getFiscalPeriodByFiscalWeek(i)) && shippedPeriodList.Contains(fiscalYear * 100 + CommonUtil.getFiscalPeriodByFiscalWeek(i)))) //  first week of shipped period & shipped period columns
                            {
                                ...
                                // todo: reset pl & bs values if shipped in current year

                                NSLedCostDataByWeekDef costByWeekDef = AccountManager.Instance.getNSLedCostDataByWeek(def.ItemNo, fiscalYear, CommonUtil.getFiscalPeriodByFiscalWeek(i), this.getPeriodLastWeekNo(CommonUtil.getFiscalPeriodByFiscalWeek(i)), def.SeasonId, 1);

                                lastPnL = lastComm
                                            - ((costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m)) / costByWeekDef.Qty * lastNetQty)
                                            + (costByWeekDef.SettlementDiscountAmtInUSD / costByWeekDef.Qty * lastNetQty)
                                            - ((costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m) - costByWeekDef.SettlementDiscountAmtInUSD) * stockProvisionRate);

                                lastBS = (((costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m)) - ((costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m)) / costByWeekDef.Qty * lastNetQty)) * -1)  // stock
                                     + (costByWeekDef.SettlementDiscountAmtInUSD - (costByWeekDef.SettlementDiscountAmtInUSD / costByWeekDef.Qty * lastNetQty)) // settlement discount
                                     + ((costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m) - costByWeekDef.SettlementDiscountAmtInUSD) * stockProvisionRate);

                                lastStock = (costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m)) * -1;
                                lastProvision = (costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m) - costByWeekDef.SettlementDiscountAmtInUSD) * -1 * stockProvisionRate;
                                lastSettlementDiscountForStock = costByWeekDef.SettlementDiscountAmtInUSD;


                                if (firstStockCol == 0)
                                    firstStockCol = currentCol;

                                this.updateBookedStockValue(document, currentSheetId, itemStartingRow, string.Format("{0}{1}", OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 11).ToString()));
                                this.updateBookedSettlementDiscountValue(document, currentSheetId, itemStartingRow, string.Format("{0}{1}", OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 12).ToString()));
                    ...
                    else //Summary not entered
                    {
                        ...
                        */
                        ...
                        foreach (int i in summaryCol)
                        {
                            ...
                            //20210824
                            if (i > 0 && (def.ShippedYear < fiscalYear || (def.ShippedYear == fiscalYear && i >= getPeriodColumn(def.ShippedPeriod))))
                            {
                ...
                else
                {
                    ...
                    {
                        NSLedCostDataByWeekDef costByWeekDef = null;
                        int adjRequired = 0;
                        ...
                        if (i < startWeek && lastComm == 0)
                        {
                            ...
                            {
                                // todo: reset pl & bs values if shipped in current year
                                costByWeekDef = AccountManager.Instance.getNSLedCostDataByWeek(def.ItemNo, fiscalYear, CommonUtil.getFiscalPeriodByFiscalWeek(i), this.getPeriodLastWeekNo(CommonUtil.getFiscalPeriodByFiscalWeek(i)), def.SeasonId, 1);
                                lastPnL = lastComm
                                            - ((costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m)) / costByWeekDef.Qty * lastNetQty)
                                            + (costByWeekDef.SettlementDiscountAmtInUSD / costByWeekDef.Qty * lastNetQty)
                                            - ((costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m) - costByWeekDef.SettlementDiscountAmtInUSD) * stockProvisionRate);
                                ...
                                adjRequired = costByWeekDef.AdjRequired;
                            ...
                            if (isContainBFData || (def.ShippedYear < fiscalYear || (def.ShippedYear == fiscalYear && currentCol >= this.getPeriodFirstWeekColumn(def.ShippedPeriod))))
                            //20210824
                            //if (isContainBFData || (currentCol >= this.getPeriodFirstWeekColumn(def.ShippedPeriod)))
                            {
                                /*
                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 9, currentCol, lastPnL.ToString(), CellValues.Number);
                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 15, currentCol, lastBS.ToString(), CellValues.Number);
                                */
                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 9, currentCol, string.Format("={0}{1}+{2}{3}", OpenXmlUtil.getColumnLetter(currentCol - 1), (itemStartingRow + 9).ToString(), OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 8).ToString()), CellValues.Number);
                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 15, currentCol, string.Format("={0}{1}+{2}{3}", OpenXmlUtil.getColumnLetter(currentCol - 1), (itemStartingRow + 15).ToString(), OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 14).ToString()), CellValues.Number);
                            }

                            currentCol++;
                            if (summaryCol.Contains(currentCol + 1)) //hit adj col
                            {
                                if (1 == 0) // no release lock, and therefore not showing adj section 2021-07-26
                                {
                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 1, currentCol, "0", CellValues.Number);
                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 2, currentCol, "0", CellValues.Number);

                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 3, currentCol, string.Format("=-(${0}${1}*(SUM({2}{3}:{4}{3})))-(SUM({2}{5}:{4}{5}))", OpenXmlUtil.getColumnLetter(4), (itemStartingRow + 7).ToString(), OpenXmlUtil.getColumnLetter(currentCol - getNoOfWeekByPeriodColumn(currentCol + 1) + 1).ToString(), (itemStartingRow + 1).ToString(), OpenXmlUtil.getColumnLetter(currentCol - 1), (itemStartingRow + 3).ToString()), CellValues.Number);
                                    ...
                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 6, currentCol, string.Format("=-(${0}${1}*(SUM({2}{3}:{4}{3})))-(SUM({2}{5}:{4}{5}))", OpenXmlUtil.getColumnLetter(4), (itemStartingRow + 8).ToString(), OpenXmlUtil.getColumnLetter(currentCol - getNoOfWeekByPeriodColumn(currentCol + 1) + 1).ToString(), (itemStartingRow + 1).ToString(), OpenXmlUtil.getColumnLetter(currentCol - 1), (itemStartingRow + 6).ToString()), CellValues.Number);

                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 8, currentCol, string.Format("=({0}{1}+{0}{2}+{0}{3})", OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 4).ToString(), (itemStartingRow + 6).ToString(), (itemStartingRow + 7).ToString()), CellValues.Number);

                                    if (firstStockCol > 0)
                                    {
                                        OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 7, currentCol, string.Format("=-{0}{1}", OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 13).ToString()), CellValues.Number);
                                        OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 11, currentCol, string.Format("=(({0}{1})*-1)-{2}{3}-{4}{5}", OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 3).ToString(), OpenXmlUtil.getColumnLetter(2), (itemStartingRow + 10).ToString(), OpenXmlUtil.getColumnLetter(2), (itemStartingRow + 7).ToString()), CellValues.Number);
                                        OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 12, currentCol, string.Format("=(({0}{1})*-1)-{2}{3}-{4}{5}", OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 6).ToString(), OpenXmlUtil.getColumnLetter(2), (itemStartingRow + 11).ToString(), OpenXmlUtil.getColumnLetter(2), (itemStartingRow + 8).ToString()), CellValues.Number);
                                        OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 13, currentCol, string.Format("=-{0}{1}+SUM({2}{3}:{2}{4})*{5}", OpenXmlUtil.getColumnLetter(firstStockCol), (itemStartingRow + 13).ToString(), OpenXmlUtil.getColumnLetter(2), (itemStartingRow + 7).ToString(), (itemStartingRow + 8).ToString(), stockProvisionRate.ToString()), CellValues.Number);
                                    }
                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 14, currentCol, string.Format("=SUM({0}{1}:{0}{2})", OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 11).ToString(), (itemStartingRow + 13).ToString()), CellValues.Number);
                            ...
                            if (summaryCol.Contains(currentCol)) //Skip for P summary
                            {
                                ...
                                */
                                ...
                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 15, currentCol, string.Format("={0}", OpenXmlUtil.getColumnLetter(currentCol - 1) + (itemStartingRow + 15).ToString()), CellValues.Number);
                                ...
                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 8, currentCol, string.Format("=SUM({0}{2}:{1}{2})", OpenXmlUtil.getColumnLetter(currentCol - getNoOfWeekByPeriodColumn(currentCol)), OpenXmlUtil.getColumnLetter(currentCol - 1), (itemStartingRow + 8).ToString()), CellValues.Number);
                                ...
                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 14, currentCol, string.Format("=SUM({0}{2}:{1}{2})", OpenXmlUtil.getColumnLetter(currentCol - getNoOfWeekByPeriodColumn(currentCol)), OpenXmlUtil.getColumnLetter(currentCol - 1), (itemStartingRow + 14).ToString()), CellValues.Number);
                        ...
                        {
                            int tempCol = currentCol - 1; //for check if summary entered
                            int nextSummaryCol = summaryCol.Where(x => x >= currentCol - 1).Min();

                            // fill cumulatives for week columns
                            if (currentCol < nextSummaryCol)
                            {
                                for (int k = currentCol; k < nextSummaryCol; k++)
                                {
                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 9, k, string.Format("={0}{1}+{2}{3}", OpenXmlUtil.getColumnLetter(k - 1), (itemStartingRow + 9).ToString(), OpenXmlUtil.getColumnLetter(k), (itemStartingRow + 8).ToString()), CellValues.Number);
                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 15, k, string.Format("={0}{1}+{2}{3}", OpenXmlUtil.getColumnLetter(k - 1), (itemStartingRow + 15).ToString(), OpenXmlUtil.getColumnLetter(k), (itemStartingRow + 14).ToString()), CellValues.Number);
                                }
                            }
                            ...
                            */
                            ...
                            costByWeekDef = AccountManager.Instance.getNSLedCostDataByWeek(def.ItemNo, fiscalYear, CommonUtil.getFiscalPeriodByFiscalWeek(i), this.getPeriodLastWeekNo(CommonUtil.getFiscalPeriodByFiscalWeek(i)), def.SeasonId, 0);


                            int adjCol = currentCol - 1; //hit adj col
                            
                            if (costByWeekDef != null && costByWeekDef.AdjRequired == 1)
                            {
                                // adj begin
                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 1, adjCol, "0", CellValues.Number);
                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 2, adjCol, "0", CellValues.Number);

                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 3, adjCol, string.Format("=-(${0}${1}*(SUM({2}{3}:{4}{3})))-(SUM({2}{5}:{4}{5}))", OpenXmlUtil.getColumnLetter(4), (itemStartingRow + 7).ToString(), OpenXmlUtil.getColumnLetter(adjCol - getNoOfWeekByPeriodColumn(adjCol + 1) + 1).ToString(), (itemStartingRow + 1).ToString(), OpenXmlUtil.getColumnLetter(adjCol - 1), (itemStartingRow + 3).ToString()), CellValues.Number);

                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 4, adjCol, string.Format("={0}{1}+{0}{2}", OpenXmlUtil.getColumnLetter(adjCol), (itemStartingRow + 2).ToString(), (itemStartingRow + 3).ToString()), CellValues.Number);
                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 5, adjCol, string.Format("={0}{1}/{0}{2}", OpenXmlUtil.getColumnLetter(adjCol), (itemStartingRow + 4).ToString(), (itemStartingRow + 2).ToString()), CellValues.Number);

                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 6, adjCol, string.Format("=-(${0}${1}*(SUM({2}{3}:{4}{3})))-(SUM({2}{5}:{4}{5}))", OpenXmlUtil.getColumnLetter(4), (itemStartingRow + 8).ToString(), OpenXmlUtil.getColumnLetter(adjCol - getNoOfWeekByPeriodColumn(adjCol + 1) + 1).ToString(), (itemStartingRow + 1).ToString(), OpenXmlUtil.getColumnLetter(adjCol - 1), (itemStartingRow + 6).ToString()), CellValues.Number);

                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 8, adjCol, string.Format("=({0}{1}+{0}{2}+{0}{3})", OpenXmlUtil.getColumnLetter(adjCol), (itemStartingRow + 4).ToString(), (itemStartingRow + 6).ToString(), (itemStartingRow + 7).ToString()), CellValues.Number);

                                if (firstStockCol > 0)
                                {
                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 7, adjCol, string.Format("=-{0}{1}", OpenXmlUtil.getColumnLetter(adjCol), (itemStartingRow + 13).ToString()), CellValues.Number);
                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 11, adjCol, string.Format("=(({0}{1})*-1)-{2}{3}-{4}{5}", OpenXmlUtil.getColumnLetter(adjCol), (itemStartingRow + 3).ToString(), OpenXmlUtil.getColumnLetter(2), (itemStartingRow + 10).ToString(), OpenXmlUtil.getColumnLetter(2), (itemStartingRow + 7).ToString()), CellValues.Number);
                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 12, adjCol, string.Format("=(({0}{1})*-1)-{2}{3}-{4}{5}", OpenXmlUtil.getColumnLetter(adjCol), (itemStartingRow + 6).ToString(), OpenXmlUtil.getColumnLetter(2), (itemStartingRow + 11).ToString(), OpenXmlUtil.getColumnLetter(2), (itemStartingRow + 8).ToString()), CellValues.Number);
                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 13, adjCol, string.Format("=-{0}{1}+SUM({2}{3}:{2}{4})*{5}", OpenXmlUtil.getColumnLetter(firstStockCol), (itemStartingRow + 13).ToString(), OpenXmlUtil.getColumnLetter(2), (itemStartingRow + 7).ToString(), (itemStartingRow + 8).ToString(), stockProvisionRate.ToString()), CellValues.Number);
                                }

                                OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 14, adjCol, string.Format("=SUM({0}{1}:{0}{2})", OpenXmlUtil.getColumnLetter(adjCol), (itemStartingRow + 11).ToString(), (itemStartingRow + 13).ToString()), CellValues.Number);


                                // adj end
                            }

                            OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 9, adjCol, string.Format("={0}{1}+{2}{3}", OpenXmlUtil.getColumnLetter(adjCol - 1), (itemStartingRow + 9).ToString(), OpenXmlUtil.getColumnLetter(adjCol), (itemStartingRow + 8).ToString()), CellValues.Number);
                            OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 15, adjCol, string.Format("={0}{1}+{2}{3}", OpenXmlUtil.getColumnLetter(adjCol - 1), (itemStartingRow + 15).ToString(), OpenXmlUtil.getColumnLetter(adjCol), (itemStartingRow + 14).ToString()), CellValues.Number);
                            ...
                            foreach (int j in summaryCol)
                            {
                                if (j > currentCol)
                                {
                                    ...
                                    */
                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 9, j, string.Format("={0}{1}", OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 9).ToString()), CellValues.Number);
                                    OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 15, j, string.Format("={0}{1}", OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 15).ToString()), CellValues.Number);
                                }
                            }
                        ...
                        if (!string.IsNullOrEmpty(sales.ItemNo))
                            AccountManager.Instance.fillNSLedCostDataByWeek(sales, def.SeasonId);
                        ...
                        //OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 3, currentCol, string.Format("={0}*{1}*-1", OpenXmlUtil.getColumnLetter(4) + (itemStartingRow + 7).ToString(), OpenXmlUtil.getColumnLetter(currentCol) + (itemStartingRow + 1).ToString()), CellValues.Number);
                        // 2021-06-17 consume sales info cost data
                        OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 3, currentCol, string.Format("=({0}+({3}*{2})+({0}*{4}/100))/{2}*{1}*-1", sales.FOBAmtInUSD.ToString(), OpenXmlUtil.getColumnLetter(currentCol) + (itemStartingRow + 1).ToString(), OpenXmlUtil.getColumnLetter(4) + (itemStartingRow + 2).ToString(), sales.ActualFreightCostPerUnitInUSD.ToString(), sales.DutyPercent.ToString()), CellValues.Number);
                        ...
                        //OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 6, currentCol, string.Format("={0}*{1}*-1", OpenXmlUtil.getColumnLetter(4) + (itemStartingRow + 8).ToString(), OpenXmlUtil.getColumnLetter(currentCol) + (itemStartingRow + 1).ToString()), CellValues.Number);
                        // 2021-06-17 consume sales info cost data
                        OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 6, currentCol, string.Format("={0}/{2}*{1}", sales.SettlementDiscountAmtInUSD.ToString(), OpenXmlUtil.getColumnLetter(currentCol) + (itemStartingRow + 1).ToString(), OpenXmlUtil.getColumnLetter(4) + (itemStartingRow + 2).ToString()), CellValues.Number);
                        ...
                        */
                        ...
                        lastPnL += (
                                    sales.NSCommAmtInUSD
                                    - ((sales.FOBAmtInUSD + (sales.ActualFreightCostPerUnitInUSD * def.TotalQty) + (sales.FOBAmtInUSD * sales.DutyPercent / 100.0m)) / def.TotalQty * sales.NetQty)
                                    + (sales.SettlementDiscountAmtInUSD / def.TotalQty * sales.NetQty));

                        // 2021-07-26 handle multiple shipments

                        //if ((def.ShippedYear == fiscalYear && currentCol == this.getPeriodFirstWeekColumn(CommonUtil.getFiscalPeriodByFiscalWeek(i)) && shippedPeriodList.Contains(fiscalYear * 100 + CommonUtil.getFiscalPeriodByFiscalWeek(i)))) //  first week of shipped period & shipped period columns
                        if ((currentCol == this.getPeriodFirstWeekColumn(CommonUtil.getFiscalPeriodByFiscalWeek(i)) && shippedPeriodList.Contains(fiscalYear * 100 + CommonUtil.getFiscalPeriodByFiscalWeek(i)))) //  first week of shipped period & shipped period columns
                        {
                            // todo: reset pl & bs values if shipped in current year
                            costByWeekDef = AccountManager.Instance.getNSLedCostDataByWeek(def.ItemNo, fiscalYear, CommonUtil.getFiscalPeriodByFiscalWeek(i), this.getPeriodLastWeekNo(CommonUtil.getFiscalPeriodByFiscalWeek(i)), def.SeasonId, 1);
                        }
                        ...
                        {
                            // todo: reset pl & bs values if shipped in current year
                            lastPnL += ((costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m)) / costByWeekDef.Qty * 0)
                                        + (costByWeekDef.SettlementDiscountAmtInUSD / costByWeekDef.Qty * 0)
                                        - ((costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m) - costByWeekDef.SettlementDiscountAmtInUSD) * stockProvisionRate);
                        ...
                        */
                        ...
                        //if ((!isContainBFData && def.ShippedYear == fiscalYear && currentCol == this.getPeriodFirstWeekColumn(CommonUtil.getFiscalPeriodByFiscalWeek(i)) && shippedPeriodList.Contains(fiscalYear * 100 + CommonUtil.getFiscalPeriodByFiscalWeek(i)))) //  first week of shipped period
                        if ((!isContainBFData && currentCol == this.getPeriodFirstWeekColumn(CommonUtil.getFiscalPeriodByFiscalWeek(i)) && shippedPeriodList.Contains(fiscalYear * 100 + CommonUtil.getFiscalPeriodByFiscalWeek(i)))) //  first week of shipped period
                        {
                            this.updateBookedStockValue(document, currentSheetId, itemStartingRow, string.Format("{0}", lastStock.ToString()));
                            this.updateBookedSettlementDiscountValue(document, currentSheetId, itemStartingRow, string.Format("{0}", lastSettlementDiscountForStock.ToString()));
                        ...
                        */
                        // 2021-06-17 consume sales info cost data
                        lastBS += (
                            ((sales.FOBAmtInUSD + (sales.ActualFreightCostPerUnitInUSD * def.TotalQty) + (sales.FOBAmtInUSD * sales.DutyPercent / 100.0m)) / def.TotalQty * sales.NetQty)  // stock
                            + (sales.SettlementDiscountAmtInUSD / def.TotalQty * sales.NetQty * -1)
                            );
                        ...
                        {
                            // todo: reset pl & bs values if shipped in current year
                            lastBS += (((costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m)) - ((costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m)) / costByWeekDef.Qty * 0)) * -1)  // stock
                                    + (costByWeekDef.SettlementDiscountAmtInUSD - (costByWeekDef.SettlementDiscountAmtInUSD / costByWeekDef.Qty * 0)) // settlement discount
                                    + ((costByWeekDef.FOBAmtInUSD + (costByWeekDef.ActualFreightCostPerUnitInUSD * costByWeekDef.Qty) + (costByWeekDef.FOBAmtInUSD * costByWeekDef.DutyPercent / 100.0m) - costByWeekDef.SettlementDiscountAmtInUSD) * stockProvisionRate);
                        }
                        ...
                        */
                        ...
                        if (summaryCol.Contains(currentCol))
                        {
                            ...
                            OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 2, currentCol, weekComm.ToString(), CellValues.Number);
                            //OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 3, currentCol, string.Format("={0}*{1}*-1", OpenXmlUtil.getColumnLetter(4) + (itemStartingRow + 7).ToString(), OpenXmlUtil.getColumnLetter(currentCol) + (itemStartingRow + 1).ToString()), CellValues.Number);
                            OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 3, currentCol, string.Format("=SUM({0}{2}:{1}{2})", OpenXmlUtil.getColumnLetter(currentCol - getNoOfWeekByPeriodColumn(currentCol)), OpenXmlUtil.getColumnLetter(currentCol - 1), (itemStartingRow + 3).ToString()), CellValues.Number);

                            OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 4, currentCol, string.Format("={0}{1}+{0}{2}", OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 2).ToString(), (itemStartingRow + 3).ToString()), CellValues.Number);
                            OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 5, currentCol, string.Format("={0}{1}/{0}{2}", OpenXmlUtil.getColumnLetter(currentCol), (itemStartingRow + 4).ToString(), (itemStartingRow + 2).ToString()), CellValues.Number);

                            //OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 6, currentCol, string.Format("={0}*{1}*-1", OpenXmlUtil.getColumnLetter(4) + (itemStartingRow + 8).ToString(), OpenXmlUtil.getColumnLetter(currentCol) + (itemStartingRow + 1).ToString()), CellValues.Number);
                            OpenXmlUtil.setCellValue(document, currentSheetId, itemStartingRow + 6, currentCol, string.Format("=SUM({0}{2}:{1}{2})", OpenXmlUtil.getColumnLetter(currentCol - getNoOfWeekByPeriodColumn(currentCol)), OpenXmlUtil.getColumnLetter(currentCol - 1), (itemStartingRow + 6).ToString()), CellValues.Number);
