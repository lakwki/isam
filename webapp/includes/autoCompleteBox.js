...
function actb_onblur(obj,evt,mCbId){
	var cbObj=document.all[mCbId];

	if (cbObj.item(cbObj.selectedIndex).text!=obj.value)
		obj.value=cbObj.item(cbObj.selectedIndex).text;
}

var actb_removeTimeoutId=null;

function actb(obj, evt, mCbId, isFirstText){
	/* ---- Variables ---- */
	var actb_timeOut = -1;//5*1000; // Autocomplete Timeout in ms (-1: autocomplete never time out)
	var actb_lim = 10;    // Number of elements autocomplete can show (-1: no limit)
	var actb_firstText = isFirstText; // should the auto complete be limited to the beginning of keyword?
	...
	var cbObj=document.all[mCbId];
	var ca=new Array(cbObj.length);
	var cbId=mCbId

	if (actb_removeTimeoutId) {
		clearTimeout(actb_removeTimeoutId);
		actb_removeTimeoutId=null;
	}

	for(var i=0; i<cbObj.length; i++){
		ca[i]=cbObj.item(i).text;
	}
	if (obj.value==cbObj.item(cbObj.selectedIndex).text)
		obj.value="";
	...
	function actb_clear(evt){
		...
		actb_removedisp();
		actb_onblur(actb_curr, evt, cbId);
	}
	...
	function actb_generate(){
		...
		for (i=0;i<actb_keywords.length;i++){
			...
			if (j - 1 == actb_lim && j < actb_total){
				r = a.insertRow(-1);
				r.style.backgroundColor = actb_aBgColor;
				c = r.insertCell(-1);
				c.style.color = actb_textColor;
	...
	function actb_table_unfocus(){
		actb_mouse_on_list = 0;
		if (actb_toid) clearTimeout(actb_toid);
		actb_toid = setTimeout(function(){actb_mouse_on_list = 0;actb_removedisp();},2000);
	}
	...
	function actb_penter(){
		...
		document.all[cbId].selectedIndex=selectedIndex;
		actb_insertword(word);
		actb_callEvent();
		actb_showAllSelects();
		document.all[cbId].fireEvent("onchange");
	}
	function actb_callEvent(){
		if (cbObj.onClientChange) {
			eval(cbObj.onClientChange)
		}
	}
...
function actb_switchPanel(single, multiple, isMultiple){
