...
INNER JOIN [RangePlanLatestVersionId] v ON r.RangePlanId = v.RangePlanId AND r.WorkflowStatusId NOT IN (60) AND r.VersionId = v.VersionId AND (r.SeasonId = -1 OR (-1 = -1 AND IsRepeatItem = 0)) 
AND (r.OfficeId IN (1,2,3,4,6,7,8,9,10,13,14,15,16,17,18,19)) 
AND ('' = '' OR r.ItemNo = '') 
...
order by MAX(r.ItemNo) 


select * from rangeplan where itemno = '823848'

293232


select distinct itemno from  rangeplan where isrepeatitem = 1