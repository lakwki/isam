...
declare @enddate datetime = '2021-01-30'
...
FROM Shipment a INNER JOIN Contract b ON a.ContractId = b.ContractId AND a.Status = 1 --and a.shipmentid = 1125346
	...
	INNER JOIN Invoice c ON a.ShipmentId = c.ShipmentId AND c.InvoiceDate BETWEEN '2021-01-24' and '2021-01-30' AND c.InvoiceUploadDate >= ( @FromDate - 180 )
	and a.supplieratwarehousedate between '2020-12-27' and '2021-01-23'
...
WHERE (SELECT COUNT(*) FROM CutOffSales AS cx WITH (NOLOCK) WHERE cx.ShipmentId = a.ShipmentId) = 0
	...
	AND EXISTS(SELECt * from nuksales where shipmentid = a.shipmentid)
