...
{
	public class SunInterfaceProcessHandler : IStartupHandler
	{
		private static readonly string DEFAULT_NAME = "SunInterfaceProcessHandler";
		...
		public void runNow() 
		{
            try
            {
			    MailHelper.sendGeneralMessage("ISAM - Sun Interface Process Start...", DateTime.Now.ToString());
			    AccountManager.Instance.processQueues();
                MailHelper.sendGeneralMessage("ISAM - Sun Interface Process Completed", DateTime.Now.ToString());
            }
            catch (Exception e)
            {
		...
		{
			return SunInterfaceProcessHandler.DEFAULT_NAME;
		}
	}
}
