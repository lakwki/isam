...
CASE 
WHEN ISNULL(rpm.FiscalYear, 2021) != MIN(s.FiscalYear) THEN 
(MIN(s.FiscalYear) * 100 + 52) 
- CAST(MIN(CAST(s.FiscalYear AS varchar) + RIGHT('0' + CAST(s.FiscalWeek AS varchar),2)) AS INT) + 1 
+ 
CAST(ISNULL(rpm.WeekStr, CAST(2021 AS varchar) + RIGHT('0' + CAST(6 AS varchar),2)) AS INT) 
- (ISNULL(rpm.FiscalYear, 2021) * 100 + 1) + 1 
+ (ISNULL(rpm.FiscalYear, 2021) - MIN(s.FiscalYear) - 1) * 52 
ELSE 
CAST(ISNULL(rpm.WeekStr, CAST(2021 AS varchar) + RIGHT('0' + CAST(6 AS varchar),2)) AS INT) 
- 
...
CASE 
WHEN ISNULL(ISNULL(md.FiscalYear, rpm.FiscalYear), 2021) != MIN(s.FiscalYear) THEN 
(MIN(s.FiscalYear) * 100 + 52) 
- CAST(MIN(CAST(s.FiscalYear AS varchar) + RIGHT('0' + CAST(s.FiscalWeek AS varchar),2)) AS INT) + 1 
+ 
CAST(ISNULL(ISNULL(md.LastFPWeekStr, rpm.WeekStr), CAST(2021 AS varchar) + RIGHT('0' + CAST(6 AS varchar),2)) AS INT) 
- (ISNULL(ISNULL(md.FiscalYear, rpm.FiscalYear), 2021) * 100 + 1) + 1 
+ (ISNULL(ISNULL(md.FiscalYear, rpm.FiscalYear), 2021) - MIN(s.FiscalYear) - 1) * 52 
ELSE 
CAST(ISNULL(ISNULL(md.LastFPWeekStr, rpm.WeekStr), CAST(2021 AS varchar) + RIGHT('0' + CAST(6 AS varchar),2)) AS INT) 
- 
CAST(MIN(CAST(s.FiscalYear AS varchar) + RIGHT('0' + CAST(s.FiscalWeek AS varchar),2)) AS INT) 
...
CONVERT(int, RIGHT(MIN(CAST(s.FiscalYear AS varchar) + RIGHT('0' + CAST(s.FiscalWeek AS varchar),2)), 2)) AS LaunchWeek, -1 AS ShippedPeriod 
...
AND s.OfficeId IN (1,2,3,4,6,7,8,9,10,13,14,15,16,17,18,19) 
AND (s.FiscalYear * 100 + s.FiscalWeek) <= (2021 * 100 + 6) 
...
AND [NS-LED].dbo.fn_getNSLedPhaseId(rp.ActualSaleSeasonId, rp.ActualSaleSeasonSplitId) IN (9,8,7,6,5,4,3,2,1) 
...
--This is the update for ultimate week count from the start of the product sell to selected week. 
CASE 
WHEN 2021 != MIN(s.FiscalYear) THEN 
(MIN(s.FiscalYear) * 100 + 52) 
- CAST(MIN(CAST(s.FiscalYear AS varchar) + RIGHT('0' + CAST(s.FiscalWeek AS varchar),2)) AS INT) + 1 
+ 
CAST(CAST(2021 AS varchar) + RIGHT('0' + CAST(6 AS varchar),2) AS INT) 
- (2021 * 100 + 1) + 1 
+ (2021 - MIN(s.FiscalYear) - 1) * 52 
ELSE 
CAST(CAST(2021 AS varchar) + RIGHT('0' + CAST(6 AS varchar),2) AS INT) 
- 
CAST(MIN(CAST(s.FiscalYear AS varchar) + RIGHT('0' + CAST(s.FiscalWeek AS varchar),2)) AS INT) 
...
CASE 
WHEN ISNULL(md.FiscalYear, 2021) != MIN(s.FiscalYear) THEN 
(MIN(s.FiscalYear) * 100 + 52) 
- CAST(MIN(CAST(s.FiscalYear AS varchar) + RIGHT('0' + CAST(s.FiscalWeek AS varchar),2)) AS INT) + 1 
+ 
CAST(ISNULL(md.LastFPWeekStr, CAST(2021 AS varchar) + RIGHT('0' + CAST(6 AS varchar),2)) AS INT) 
- (ISNULL(md.FiscalYear, 2021) * 100 + 1) + 1 
+ (ISNULL(md.FiscalYear, 2021) - MIN(s.FiscalYear) - 1) * 52 
ELSE 
CAST(ISNULL(md.LastFPWeekStr, CAST(2021 AS varchar) + RIGHT('0' + CAST(6 AS varchar),2)) AS INT) 
- 
CAST(MIN(CAST(s.FiscalYear AS varchar) + RIGHT('0' + CAST(s.FiscalWeek AS varchar),2)) AS INT) 
...
UNION ALL 
SELECT c.ItemNo, a.OfficeId, 0 AS IsEndOfLife, 0 AS WeekCount, 0 AS FPWeekCount, 0 AS QtySold, 0 AS FPQty, 0 AS UKQty, 0 AS TotalNSCommissionAmt, 0 AS InitialSaleWeek, 0 AS SeasonCount, 0 AS SeasonId, 
0 AS HasDuty, 0 AS LaunchYear, 0 AS LaunchWeek, MIN(cal.BudgetYear * 100 + cal.Period) AS ShippedPeriod 
...
AND (a.OfficeId IN (1,2,3,4,6,7,8,9,10,13,14,15,16,17,18,19)) 
...
INNER JOIN NSLDB..AccountFinancialCalender cal ON cal.AppId = 13 AND p.InvoiceDate BETWEEN cal.StartDate AND cal.EndDate AND cal.Status = 1 
LEFT JOIN NSLedSalesSummary d ON c.ItemNo = d.ItemNo 

WHERE (d.ItemNo IS NULL OR 
NOT EXISTS(SELECT * FROM NSLedSalesSummary y WHERE ItemNo = c.ItemNo AND y.[Week] <= (2021 * 100 + 6))) 
GROUP BY c.ItemNo, a.OfficeId 

UNION ALL 
SELECT c.ItemNo, a.OfficeId, 0 AS IsEndOfLife, 0 AS WeekCount, 0 AS FPWeekCount, 0 AS QtySold, 0 AS FPQty, 0 AS UKQty, 0 AS TotalNSCommissionAmt, 0 AS InitialSaleWeek, 2 AS SeasonCount, r.SeasonId AS SeasonId, 
0 AS HasDuty, 0 AS LaunchYear, 0 AS LaunchWeek, MIN(cal.BudgetYear * 100 + cal.Period) AS ShippedPeriod 
...
WHERE (d.ItemNo IS NULL OR 
NOT EXISTS(SELECT * FROM NSLedSalesSummary y WHERE ItemNo = c.ItemNo AND y.[Week] <= (2021 * 100 + 6) AND y.[Week] >= (r.FiscalYear * 100 + r.FiscalWeek))) 
GROUP BY c.ItemNo, a.OfficeId, r.SeasonId 
) z 
--WHERE z.ItemNo IN ('142329', '704485', '499341', '997410','107335','292573') 
--WHERE z.ItemNo IN ('142329') 
