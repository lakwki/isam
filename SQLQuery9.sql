...
+ 
CAST(CAST(2020 AS varchar) + RIGHT('0' + CAST(37 AS varchar),2) AS INT) 
- (2020 * 100 + 1) + 1 
...
- CAST(MIN(CAST(f.FiscalYear AS varchar) + RIGHT('0' + CAST(f.FiscalWeek AS varchar),2)) AS INT) + 1 
+ 
CAST(ISNULL(md.WeekStr, CAST(2020 AS varchar) + RIGHT('0' + CAST(37 AS varchar),2)) AS INT) 
- (ISNULL(CONVERT(int,LEFT(md.WeekStr,4)), 2020) * 100 + 1) + 1 
...
ISNULL(rip.SeasonId, 0) > 0 THEN 2 ELSE 1 END AS SeasonCount, 
ISNULL(rip.SeasonId, 0) AS SeasonId, 
...
WHERE (f.OfficeId IN (1,2,3,4,6,7,8,9,10,13,14,15,16,17,18,19)) -- AND s.ItemNo = '643807' AND f.FiscalYear = 2020 AND f.FiscalWeek = 37 
) temp ON temp.ItemNo = s.ItemNo AND (s.DespatchQty <> 0 OR s.ReturnQty <> 0) 
INNER JOIN [NSLedImportFile] f ON s.FileId = f.FileId 
AND (f.FiscalYear * 100 + f.FiscalWeek) <= (2020 * 100 + 37) 
/* 
AND CAST(f.FiscalYear AS varchar) + RIGHT('0' + CAST(f.FiscalWeek AS varchar),2) <= CAST(2020 AS varchar) + RIGHT('0' + CAST(37 AS varchar),2) 
*/ 

LEFT JOIN vw_FirstNSLedMDWeek md ON md.ItemNo = s.ItemNo AND f.OfficeId = md.OfficeId 
LEFT JOIN (SELECT TOP 1 * FROM NSLedRepeatItemParam WHERE s.ItemNo = ItemNo AND (f.FiscalYear * 100 + f.FiscalWeek) <= (FiscalYear * 100 + FiscalWeek) ORDER BY FiscalYear, FiscalWeek) rip 
...
GROUP BY s.ItemNo, f.OfficeId, ISNULL(rip.SeasonId, 0), md.WeekStr 
...
NOT EXISTS(SELECT * FROM NSLedSales x INNER JOIN NSLedImportFile y ON x.ItemNo = d.ItemNo AND x.FileId = y.FileId AND (y.FiscalYear*100 + y.FiscalWeek) <= (2020 * 100 + 37))) 
) z 
--WHERE z.ItemNo IN ('124711','383184','818469', '295108') 
ORDER BY OfficeSortId, IsEndOfLife DESC, WeekCount DESC, ItemNo 


select * from nsldb..season
